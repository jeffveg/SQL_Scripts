DECLARE @TableName NVARCHAR(200) = N'JeffTestPartition'
...
LEFT OUTER JOIN sys.partition_range_values rv
	 ...
	 JOIN sys.database_files df
            ON df.data_space_id = au.filegroup_id
WHERE i.index_id < 2
     AND o.object_id = OBJECT_ID(@TableName)
		...
		WITH    TablePartitions(TableName, PartNum, TableFileGroup, Comparison, Value)
          AS (SELECT    SCHEMA_NAME(o.schema_id) + '.'
     ...
     --, au.total_pages AS pages
              ...
              WHERE     i.index_id < 2
                        ...
                        AND au.type = 1
                        AND fg.name <> 'TINY')
    SELECT   TableFileGroup
    FROM    TablePartitions
    WHERE   PartNum = (SELECT   MAX(PartNum)
                       FROM     TablePartitions);
