...
SET @BackUpPath = 'T:\Backup\Microsoft SQL Server\MSSQL10_50.PROD2008\MSSQL\Backup\'
...
BEGIN
	SET @Statment = 'EXECUTE master.dbo.xp_delete_file 0,N''' + @BackUpPath + @DBName + ''',N''trn'',N''' + @DTDelete + ''',1'
	EXEC sp_executeSQL @Statment

	SET @Statment = 'EXECUTE master.dbo.xp_create_subdir ''' + @BackUpPath + @DBName + ''''
	EXEC sp_executeSQL @Statment

	SET @Statment =  'BACKUP LOG [' + @DBName + '] TO  DISK = ''' + @BackUpPath + @DBName + '\' + @DBName + '_backup_' + @DTStamp +'.trn'' WITH NOFORMAT, NOINIT, NAME = ''' + @DBName + 'backup_' + @DTStamp + ''', SKIP, REWIND, NOUNLOAD, STATS = 10'
	EXEC sp_executeSQL @Statment
