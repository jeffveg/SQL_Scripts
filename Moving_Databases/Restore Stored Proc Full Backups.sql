...
IF EXISTS (
		SELECT 1
		FROM sys.objects
		WHERE NAME = 'usp_RestoreFullForMove'
		)
	DROP PROCEDURE usp_RestoreFullForMove
GO

CREATE PROCEDURE [dbo].[usp_RestoreFullForMove] (
	@DBName VARCHAR(255)
...
SET @RemoteBAKFileName = @RemotePath + @Dbname + '.bak'
SET @LocalBAKFileName = @LocalPath + @Dbname + '.bak'
...
SET @SQL = @SQL + '   WITH NORECOVERY, ' + @CRLF
