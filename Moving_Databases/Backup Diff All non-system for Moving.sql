...
BEGIN
	IF (@@fetch_status <> - 2)
	BEGIN
		SET @Message = 'Differential Backup for database ' + @DBName
		...
		SET @SQL = @SQL + '  TO DISK = ''' + @BackUpPath + @DBName + '_Diff.bak''' + @CRLF
		SET @SQL = @SQL + 'WITH ' + @CRLF
		SET @SQL = @SQL + '  DIFFERENTIAL ' + @CRLF
		SET @SQL = @SQL + ' ,NOFORMAT ' + @CRLF
		SET @SQL = @SQL + ' ,NAME = N''' + @DBName + '-Diff Database Backup''' + @CRLF
